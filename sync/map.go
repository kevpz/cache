...
// Package sync provides a generic concurrent hash-trie map extracted from
// Go's internal/sync.HashTrieMap. Internal dependencies (abi, goarch, race)
// are replaced with public equivalents (hash/maphash, reflect, sync).
package sync
...
func (ht *HashTrieMap[K, V]) Load(key K) (value V, ok bool) {
	...
	panic("sync.HashTrieMap: ran out of hash bits while iterating")
}
...
func (ht *HashTrieMap[K, V]) expand(oldEntry, newEntry *entry[K, V], newHash uint64, hashShift uint, parent *indirect[K, V]) *node[K, V] {
	...
	for {
		if hashShift == 0 {
			panic("sync.HashTrieMap: ran out of hash bits while inserting")
		}
		hashShift -= nChildrenLog2
