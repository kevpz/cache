...
// Generic concurrent hash-trie map extracted from Go's internal/sync.HashTrieMap.
// Internal dependencies (abi, goarch, race) are replaced with public equivalents
// (hash/maphash, reflect, sync).
...
// --- HashTrieStore (Store implementation) ---
...
// --- HashTrieMap (generic concurrent hash-trie) ---

const htPtrSize = uint(unsafe.Sizeof((*int)(nil)))
...
type HashTrieMap[K comparable, V any] struct {
	...
	keyHash  htHashFunc[K]
	valEqual htEqualFunc[V]
	seed     maphash.Seed
}
...
type htHashFunc[K comparable] func(key K, seed maphash.Seed) uint64

type htEqualFunc[V any] func(v1, v2 V) bool
...
func (ht *HashTrieMap[K, V]) Load(key K) (value V, ok bool) {
	...
	panic("HashTrieMap: ran out of hash bits while iterating")
}
...
func (ht *HashTrieMap[K, V]) expand(oldEntry, newEntry *entry[K, V], newHash uint64, hashShift uint, parent *indirect[K, V]) *node[K, V] {
	...
	for {
		if hashShift == 0 {
			panic("HashTrieMap: ran out of hash bits while inserting")
		}
		hashShift -= nChildrenLog2
...
func (ht *HashTrieMap[K, V]) find(key K, hash uint64, valEqual htEqualFunc[V], value V) (i *indirect[K, V], hashShift uint, slot *atomic.Pointer[node[K, V]], n *node[K, V]) {
	for {
		i = ht.root.Load()
		hashShift = 8 * htPtrSize
		found := false
...
func (e *entry[K, V]) lookupWithValue(key K, value V, valEqual htEqualFunc[V]) (V, bool) {
	for e != nil {
...
func (head *entry[K, V]) compareAndSwap(key K, old, new V, valEqual htEqualFunc[V]) (*entry[K, V], bool) {
...
func (head *entry[K, V]) compareAndDelete(key K, value V, valEqual htEqualFunc[V]) (*entry[K, V], bool) {
